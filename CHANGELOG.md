...
- **Protected Profile Headshot Dimensions**: Appended strict specificity overrides to the sidebar profile picture headshot selectors (`.profile .headshot img`), ensuring the image remains a circular `100px` by `100px` card element with its custom crop alignments instead of inheriting the default post grid heights.
